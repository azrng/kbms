...
对于敏感操作，可以设置工具需要人工审批。v1.6.1 增强了工具审批，支持 "Don't ask again" 规则。

```csharp
// 方式一：通过 AIAgentBuilder
var agent = new AIAgentBuilder()
    .UseToolApproval()  // 启用工具审批
    .Build(chatClientAgent);

// 方式二：ToolApprovalAgent 装饰器
var agent = new ToolApprovalAgent(chatClientAgent, new ToolApprovalOptions
{
    // 配置审批规则
    AutoApproveRules = new Dictionary<string, bool>
    {
        ["GetWeather"] = true,       // 自动批准
        ["DeleteUser"] = false       // 必须人工审批
    }
});
...
v1.6.1 新增 Hyperlight 沙箱执行，Agent 可在安全隔离的 VM 中执行代码。
...
```xml
<PackageReference Include="Microsoft.Agents.AI.Workflows.Declarative.Mcp" Version="1.6.1" />
```
...
v1.6.1 新增深度研究模式，使用 o3 深度推理模型 + Bing grounding。
